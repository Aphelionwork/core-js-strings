...
function concatenateStrings(value1, value2) {
  const temp = value1.concat(value2);
  return temp;
}
...
function removeFirstOccurrences(str, value) {
  if (typeof str === 'string') {
    ...
    const lenghtOfValue = value.length;
    const firstPart = str.substring(0, indexOfValue);
    const secondPart = str.substring(indexOfValue + lenghtOfValue);
...
function countVowels(str) {
  // let counter = 0;
  // const vowels = ['e', 'u', 'i', 'o', 'a', 'y', 'E', 'U', 'I', 'O', 'A', 'Y'];
  // if (typeof str === 'string') {
  //   for (let i = 0; i < str.length; i += 1) {
  //     if (vowels.some(str[i]) === true) {
  //       counter += 1;
  //     }
  //   }
  // }
...
function isPalindrome(str) {
  ...
  const strLowerCase = str.toLowerCase().replace(reg, '');
  const check = strLowerCase.split('').reverse().join('');
  if (strLowerCase === check) return true;
  return false;
}
...
function getStringFromTemplate(firstName, lastName) {
  const temp = 'Hello, ';
  return temp.concat(firstName, ' ', lastName, '!');
}
...
function extractNameFromTemplate(value) {
  const regex = /, .* .*!/;
  const extract = String(value.match(regex));
  return extract.replace(/, /g, '').replace(/!/g, '');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (symbol) => originalAlphabet[cipherAlphabet.indexOf(symbol)]
  );
}
